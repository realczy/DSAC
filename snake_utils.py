...
def active_contour_step(Fu, Fv, du, dv, snake_u, snake_v, alpha, beta, kappa,
    ...
    dEb_dv = -tf.multiply(tf.reduce_sum(tf.multiply(js,
                                                    tf.gather(tf.gather(kappa_collection,
                                                                        tf.concat([tf.range(L - 1, L), tf.range(L - 1)],
                                                                                  0), axis=0),
                                                              tf.range(s - 1, -1, delta=-1), axis=1)), axis=1),
                          tf.squeeze(int_ends_u_next))
    dEb_dv += tf.multiply(tf.reduce_sum(tf.multiply(js,
                                                    tf.gather(kappa_collection,
                                                              tf.concat([tf.range(L - 1, L), tf.range(L - 1)], 0),
                                                              axis=0), ), axis=1),
    ...
    snake_u = tf.matmul(tf.matrix_inverse(tf.eye(L._value) + 2 * gamma * (A / delta_s + B / (delta_s * delta_s))),
                        snake_u + gamma * du)
    snake_v = tf.matmul(tf.matrix_inverse(tf.eye(L._value) + 2 * gamma * (A / delta_s + B / (delta_s * delta_s))),
                        snake_v + gamma * dv)
...
def plot_snakes(snake, snake_hist, GT, mapE, mapA, mapB, mapK, grads_arrayE, grads_arrayA, grads_arrayB, grads_arrayK,
                image, mask):
    # Plot result
    ...
    for i in range(0, len(snake_hist), 5):
        ax[0].plot(snake_hist[i][:, 1], snake_hist[i][:, 0], '-.',
                   color=[i / len(snake_hist), i / len(snake_hist), 1 - i / len(snake_hist)], lw=3)
    if not GT is None:
...
def CNN(im_size, out_size, L, batch_size=1, layers=5, wd=0.001, numfilt=0):
    ...
    grads = tf.gradients([predE, predA, predB, predK, l2loss], tvars,
                         grad_ys=[grad_predE, grad_predA, grad_predB, grad_predK, grad_l2loss])
